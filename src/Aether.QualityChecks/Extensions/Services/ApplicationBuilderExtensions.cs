...
namespace Aether.QualityChecks.Extensions
{
    public static class ApplicationBuilderExtensions
    {
        ...
        /// <param name="route">Optional overload for quality check route.  Defaults to "/api/qualitycheck"</param>
        /// <returns></returns>
        public static void UseFileDrivenQualityCheckMiddleware<T>(this IApplicationBuilder app, string fileName,
            string route = "/api/filedrivenqualitycheck")
        {
            ...
            if (app.ApplicationServices.GetService(typeof(IQualityCheckExecutionHandler)) == null)
            {
                throw new InvalidOperationException($"Unable to Find {nameof(IQualityCheckExecutionHandler)} - Consider using the {nameof(ServiceCollectionExtensions.RegisterFileDrivenQualityChecks)} extension method");
            }
            ...
            app.MapWhen(predicate, b => b.UseMiddleware<FileDrivenQualityCheckMiddleware>(typeof(T), fileName));
        }
        ...
        /// <param name="route">Optional overload for quality check route.  Defaults to "/api/qualitycheck"</param>
        /// <returns></returns>
        public static void UseFileDrivenQualityCheckMiddleware(this IApplicationBuilder app, string fileName,
            string route = "/api/filedrivenqualitycheck")
        {
            PathString routePs = new PathString(route);
            ...
            app.MapWhen(predicate, b => b.UseMiddleware<FileDrivenQualityCheckMiddleware>(fileName));
        }
    }
}
