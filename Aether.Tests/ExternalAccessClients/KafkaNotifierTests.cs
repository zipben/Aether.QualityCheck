...
namespace Aether.Tests.ServiceLayerTests
{
    ...
    public class KafkaNotifierTests : UnitTestBase
    {
        ...
        {
            await Assert.ThrowsExceptionAsync<ArgumentNullException>(() => _target.SendAsync(messageContent, schemaPath));
        }
        ...
        {
            await Assert.ThrowsExceptionAsync<ArgumentException>(() => _target.SendAsync(messageContent, schemaPath));
        }
        ...
        public async Task SendAsyncTest_BlankTopicSkipsSend()
        {
            ...
            kafkaSettings.Value.Topic = string.Empty;

            _target = new KafkaNotifier(_mockApiLogger.Object, kafkaSettings, _mockKafkaProducer.Object);

            SetupMocks(null);
            var response = await _target.SendAsync(CreateTestMessage(), TEST_SCHEMA);
        ...
        public async Task SendAsyncTest_InvalidSchema()
        {
            ...
            var response = await _target.SendAsync(CreateTestMessage(), TEST_INVALID_SCHEMA);
        ...
        public async Task SendAsyncTest_KafkaException()
        {
            ...
            _mockKafkaProducer.Verify(x => x.ProduceAsync(It.IsAny<string>(), It.IsAny<Message<string, string>>(), It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        public async Task SendAsyncTest(PersistenceStatus? persistenceStatus, bool expectedSuccess)
        {
            ...
            var response = await _target.SendAsync(testMessage, TEST_SCHEMA);

            Assert.AreEqual(expectedSuccess, response.Successful);

            _mockKafkaProducer.Verify(x => x.ProduceAsync(_kafkaSettings.Value.Topic, It.IsAny<Message<string, string>>(), default), Times.Once);
